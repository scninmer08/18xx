    ...
    module G18IlSolo
      ...
      def cert_limit(player = nil)
        ...
        def setup_optional_rules
          ...
          bonus = BONUS_BY_LEVEL[level] || 0
          return if bonus.zero?

          @bank.spend(bonus, human)
        end
            ...
            buckets[kind].each do |corp|
              ...
              share_sources.each do |source|
                source.call.each do |share|
                  ...
                  @bank.spend(price - @robot.cash, @robot) if @robot.cash < price
        ...
        def auto_ic_line_lay!
          t = (%w[4B 5 6 D].include?(@phase.name) ? 2 : 1)
          t.times do
            return if ic_line_completed? || @ic_line_laid_this_round || @last_set
        ...
        def solo_seed_ic_and_subs!
          ...
          sizes.zip(picks).each do |size, par_price|
            ...
            @bank.spend(par_price.price * 2, corp)
